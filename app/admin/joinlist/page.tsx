...
export default function AdminJoinlistPage() {
  ...
  const listQuery = useQuery({
    queryKey: ["admin", "joinlist", page, debouncedQ, from, to],
    queryFn: () =>
      adminFetch<PaginatedResult<JoinlistRow>>(
        `/api/admin/joinlist${buildQuery({
          page,
          limit: 20,
          q: debouncedQ,
          from,
  ...
  const columns: DataTableColumn<JoinlistRow>[] = [
  ...
  return (
    <AdminShell
      title="Joinlist"
      subtitle="App waitlist sign-ups from the homepage"
    >
      <div className="space-y-6">
        <SearchBar value={q} onChange={setQ} placeholder="Search joinlist…" />
        <FilterBar>
